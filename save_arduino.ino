...
void setupI2S() {
  i2s_config_t i2s_config = {
    ...
    .dma_buf_count = 16,     // Increased to prevent dropped frames
    .dma_buf_len = 1024,     // Increased for larger safety net
    .use_apll = false,
...
void setup() {
  ...
  tft.println("Hold BOOT to talk");

  setupI2S();
}

void loop() {
  // Check if we are connected to the Python server
  if (!client.connected()) {
    if (!client.connect(SERVER_IP, SERVER_PORT)) {
      Serial.println("Connection to server failed. Retrying in 2s...");
      delay(2000);
      return;
    }
    // Disable Nagle's Algorithm to send start/end packets instantly (Zero Lag)
    client.setNoDelay(true); 
  }

  // Check Button State (Push-to-Talk)
  ...
  // While button is held, read audio and stream it
  if (isRecording) {
    size_t bytes_read;
    ...
    // The ESP32 sends Left, Right, Left, Right. We skip by 2 to only read the Left channel.
    for (int i = 0; i < total_samples; i += 2) {
      uint16_t raw_sample = i2s_buffer[i] & 0x0FFF; 
      
      // DC Blocking Filter (Auto-centers the audio around 0)
      float x = (float)raw_sample;
      dc_filter_y = x - dc_filter_x + 0.995 * dc_filter_y;
      dc_filter_x = x;

      // Scale the volume down slightly for safety
      float scaled_audio = dc_filter_y * 12.0;

      // ANTI-ROBOTIC CLAMPER: Prevent integer overflow if audio is too loud
  ...
  // Check for incoming text from the server
  if (!isRecording && client.available()) {
    ...
    // Wrap text nicely on the TFT display
